...
const STORAGE_USUARIO = "usuarioLogado";
const STORAGE_CHECKOUT = "checkoutCarrinho";
...
const listaRevisao =
  document.getElementById("listaRevisaoPedido");

const subtotalEl =
  document.getElementById("checkoutSubtotal");

const descontoEl =
  document.getElementById("checkoutDesconto");
...
let lojaSelecionada = "Ponte Alta Norte";

let desconto = 0;

// Valor provisório do frete.
// Pode ser alterado depois conforme a regra real do projeto.
...
async function carregarCarrinho() {
  try {
    ...
    carrinhoItens = itensRaw.filter(
      (item) =>
        !Number(item.salvoParaDepois ?? 0)
    );
...
async function finalizarPedido(modal) {
  const btn =
  ...
  try {
    ...
    for (const item of carrinhoItens) {
      ...
      if (!res.ok) {
        throw new Error(
          `Não foi possível remover "${item.nomeProduto}" do carrinho.`
        );
      }
    }
...
estiloModal.textContent = `
  ...
  .modal-icone span {
    ...
    box-shadow:
      0 8px 20px rgba(39, 220, 126, 0.3);
  }
